...
def parser(loader, node):
    data = loader.construct_mapping(node)
    # choose the level according to the node tag
    lvl = {'!easy_level': EasyLevel, '!medium_level': MediumLevel, '!hard_level': HardLevel}[node.tag]
    pprint.pprint(data)
    _map = lvl.Map()
    _obj = lvl.Objects()
    _obj.config = {'enemy': data.get('enemy'), 'enemy_count': data.get('enemy_count')}
    return {'map': _map, 'obj': _obj}


loader = yaml.Loader
loader.add_constructor('!easy_level', parser)
loader.add_constructor('!medium_level', parser)
loader.add_constructor('!hard_level', parser)
